...
class IndexHandler(object):
    ...
    def func_get_information(self):
        ...
        try:
            ...
            raise response_code.ParamERR(errmsg="The parameter is of numeric type")
        res_dict = {}
        res_list = []
        video_dict = {}
        video_list = []
        ...
        res_dict["subscription_counts"] = mongo.db.subscription.find({"user_id": user["_id"], "state": 0}).count()
        view_counts = 0
        download_counts = 0
        like_counts = 0
        collections_counts = 0
        video_id_list = []
        for video in mongo.db.video.find({"user_id": user["_id"]}).sort("upload_time", -1):
        ...
        collections_counts += mongo.db.collection.find({"state": 0, "relation_id": {"$in": video_id_list}}).count()
        # 收藏
        for collection in mongo.db.collection.find({"user_id": user["_id"], "state": 0}).sort("time", -1).limit(p_collection_counts):
            if collection["type"] == "video":
                video = mongo.db.video.find_one({"_id": collection["relation_id"]})
                ...
                collection_dict["like_counts"] = mongo.db.like.find({"relation": video['_id'], "type": "video"}).count()
            elif collection["type"] == "series":
                series = mongo.db.series.find_one({"_id": collection["relation_id"]})
            ...
            {"series": series["_id"], "state": 2}).count()
                ...
                for video in mongo.db.video.find({"series": series["_id"]}):
                    ...
                    c_like_counts += mongo.db.like.find({"relation_id": video['_id'], "type": "video"}).count()
