...
from utils.common import fenye
...
class VideoHandler(object):
        ...
        if not task_id:
            ...
            if not video_info:
                ...
                try:
                    ...
                    sub_path = mongo.db.video.find_one({"_id": task_id},
                                                       {"vtt_path": 1, "_id": 0,
                                                        "ass_path": 1})
                except Exception as e:
                    current_app.log.info(e)
    ...
    def func_hot_video(self):
        ...
        try:
            ...
            video_cursor = mongo.db.video.find(
                {"state": 2, "series": {"$exists": False}}).sort("view_counts",
                                                                 -1)
            tool = mongo.db.tool.find_one({'type': 'category'}).get("data")
            for video in video_cursor:
            ...
            for series in mongo.db.series.find():
                view_counts = 0
                video_id_list = []
                ...
                series_video_cursor = mongo.db.video.find(
                    {"series": series["_id"]})
                for series_video in series_video_cursor:
                    view_counts = series_video["view_counts"]
                    video_id_list.append(series_video["_id"])
            ...
            res_sort_list = fenye(
                sorted(res_list, key=itemgetter("view_counts"), reverse=True),
                page, max_size)
    ...
    def func_hot_author(self):
        ...
        try:
            user_sub_author = []
            if user:
                user_sub_author = [i.get("relation_id") for i in
                                   mongo.db.subscription.find(
                                       {"user_id": user["_id"]},
                                       {"relation_id": 1, "_id": 1})]
            relation_id_cursor = mongo.db.subscription.find(
                {"type": "author", "relation_id": {"$nin": user_sub_author}},
                {"_id": 0,
                 "relation_id": 1}).limit(
                max_size).skip(max_size * (page - 1))
    ...
    def func_get_video(self):
        ...
        if get_type not in ["video", "series", "draft"]:
            ...
            for video in video_cursor:
                ...
                res_dict["description"] = video["description"]
            ...
            video_cursor = mongo.db.video.find(
                {"user_id": user["_id"], "state": {"$lt": 2}}).sort(
                "upload_time", -1)
            for video in video_cursor:
                like_counts = mongo.db.like.find(
                    {"relation_id": video["_id"], "type": "video"}).count()
                ...
                res_dict["state"] = video["state"]
                res_dict["image_path"] = video["image_path"]
                res_dict["description"] = video.get("description", "")
                res_dict["video_time"] = video["video_time"]
                res_dict["view_counts"] = video["view_counts"]
                res_dict["like_counts"] = like_counts
                res_dict["comment_counts"] = comment_counts
        ...
        for video_id in video_id_list:
            ...
            if "series" in video_info.keys():
                ...
                if series_video_count <= 0:
                    mongo.db.series.delete_one({"_id": series_info["_id"]})
                else:
            ...
            if document_info:
                ...
                mongo.db.rubbish.update_one({"_id": video_info["_id"]},
                                            {"$set": video_info}, upsert=True)
                mongo.db.rubbish.update_one({"_id": document_info["_id"]},
                                            {"$set": document_info},
                                            upsert=True)
                mongo.db.video.delete_one({"_id": video_info["_id"]})
                mongo.db.document.delete_one({"_id": document_info["_id"]})
            else:
                mongo.db.rubbish.update_one({"_id": video_info["_id"]},
                                            {"$set": video_info}, upsert=True)
                mongo.db.video.delete_one({"_id": video_info["_id"]})
    ...
    def func_category_information(self):
        ...
        tool = mongo.db.tool.find_one({"type": "category"}).get("data")
        if category not in [tag["id"] for tag in tool]:
        ...
        for series_id in series_distinct_list:
            ...
            res_list.append(deepcopy(series_dict))
        res_sort_list = fenye(
            sorted(res_list, key=itemgetter("view_counts"), reverse=True), page,
            max_size)
        return set_resjson(res_array=res_sort_list)
...
def upload_success_update(file_type, task_id, subtitling_list, style, lang,
    ...
    if not video_info:
        ...
        back_info = {'video_id': md5_token, "video_time": video_time,
                     'video_path': video_path, "video_size": video_size}
        ...
        resp = set_resjson(err=-1, errmsg='The video has been uploaded !')
